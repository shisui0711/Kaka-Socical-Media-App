...
import { convertToUsername, slugify } from "@/lib/utils";
...
export async function GET(req: NextRequest){
  ...
  try {
    ...
    if(existingUser){
      const sesssion = await lucia.createSession(existingUser.id, {})
    ...
    const username = convertToUsername(googleUser.name) + '-' + userId.slice(0,4)
